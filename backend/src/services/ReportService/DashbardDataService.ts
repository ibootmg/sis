...
export default async function DashboardDataService(
  ...
  counters as (
    select
      ...
      (select count(*) from traking where "status" = 'nps') "waitRating",
      (select count(*) from traking where "status" = 'closed' and "rated" = false) "withoutRating",
      (select count(*) from traking where "status" = 'closed' and "rated" = true) "withRating",
      (select 
        coalesce(
          (100.0 * count(*) filter (where tt."rated" = true)) / 
          nullif(count(*), 0), 
          0
        )
        from traking tt
      ...
      (select 
        coalesce(
          (100.0 * count(*) filter (where ur."rate" >= 9)) / 
          nullif(count(*) filter (where tt."rated" = true), 0), 
          0
        )
        from traking tt
        inner join "UserRatings" ur on ur."ticketId" = tt."ticketId"
        where tt."rated" = true and tt."status" = 'closed'
      ) "npsPromotersPerc",
      (select 
        coalesce(
          (100.0 * count(*) filter (where ur."rate" in (7, 8))) / 
          nullif(count(*) filter (where tt."rated" = true), 0), 
          0
        )
        from traking tt
      ...
      (select 
        coalesce(
          (100.0 * count(*) filter (where ur."rate" <= 6)) / 
          nullif(count(*) filter (where tt."rated" = true), 0), 
          0
        )
        from traking tt
      ...
      (select 
        coalesce(
          (100.0 * count(*) filter (where ur."rate" >= 9)) / nullif(count(*) filter (where tt."rated" = true), 0) -
          (100.0 * count(*) filter (where ur."rate" <= 6)) / nullif(count(*) filter (where tt."rated" = true), 0),
          0
        )
        from traking tt
        inner join "UserRatings" ur on ur."ticketId" = tt."ticketId"
